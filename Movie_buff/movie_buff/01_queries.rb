#actors, has many: movies, castings, directed_movies
#movies, has many: casting, actors,
#castings
...
def it_was_ok
  ...
  Movie.select(:id, :score,:title).where("score between 2 and 3")
end
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:actors).group("movies.id").order("COUNT(castings.id) DESC").limit(3)
    # SELECT 
    #   id, title
    # FROM
    #   movies
...
# [{"id"=>5, "title"=>"Star Wars: Episode V - The Empire Strikes Back"}, {"id"=>13, "title"=>"Star Wars...calypse Now"}, {"id"=>371, "title"=>"American Graffiti"}, {"id"=>700, "title"=>"Conversation, The"}]
# [{"id"=>5, "title"=>"Star Wars: Episode V - The Empire Strikes Back"}, {"id"=>5, "title"=>"Star Wars:...sation, The"}, {"id"=>700, "title"=>"Conversation, The"}, {"id"=>700, "title"=>"Conversation, The"}]
